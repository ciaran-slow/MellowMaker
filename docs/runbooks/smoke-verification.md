...
| #14 | (this PR) | `.maestro/accessibility.yaml` | Jest `useAnnouncement` (iOS announce-once, Android never, first-render silent, repeated-same-value once, return-to-value, undefined-between-identical) + `usePressScale` (both reduced-motion branches, disabled, synchronous `onPress`) + walk-based `accessibilityContrast` and `textScaling` guards + `nonColorStatus` (both step rows × 3 statuses by role/state/word) + `CraftCounter`/`PatternViewerScreen`/`GuideWorkingViewScreen` iOS announcement cases + `tabBarContrast` 3:1 indicator | **iOS VoiceOver** pass on the physical iPhone (iOS 26.6.1, Expo Go 57.0.9 via `npx expo start`) — product-owner run, recorded on the issue; **Android TalkBack** pass over dictionary → stitch detail, viewer completion + counter, import failure/success, editor save, playback seek — no Android SDK/`adb` on the build machine, and Maestro cannot launch at all (no Java runtime), so `test:smoke:accessibility` itself is also unexecuted on both platforms; real Dynamic Type / font-scale clipping and one-handed reachability on Android | Open |
