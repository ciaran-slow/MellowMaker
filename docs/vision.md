...
* **Color Palette:**
  ...
  * **Strong Accents (anything that must stay legible):** deeper companions to three of the accents — pink `#C15169`, teal `#048765`, blue `#1080A6` — used wherever text, a selection indicator, or a **fine graphic line** has to be read rather than merely seen, so it stays legible (at least 4.5:1 with white text, 3:1 against the backdrop and the white card). The bright accents keep to *large* decorative work — fills, borders, bars, and card stripes — where nothing depends on discriminating the shape. A thin stroke is the second case: the stitch step drawings are decorative in the accessibility sense (hidden from screen readers, the sentence above them carries the meaning) yet still have to clear 3:1 as non-text graphics, so they are stroked in the strong accents too, never the bright ones. Yellow already carries ink text legibly and needs no companion.
* **Shape & Typography:**
