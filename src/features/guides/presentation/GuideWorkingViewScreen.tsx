...
/**
 * The interactive guide working view — the guide's home. It shows the video
 * placeholder (the #11 player seam) above an always-rendered, always-interactive
 * step list, marks the current/next step, and lets the maker complete or reopen
 * any step with immediate durable writes. The guide's own maker-labelled counter
 * (reused unchanged from #7) is pinned above the list. Structural editing lives
 * on the child `/guides/[guideId]/edit` route, reached from "Edit guide". Like
 * the pattern viewer it owns its `FlatList` directly rather than nesting one in a
 * `ScrollView` (NFR-09).
 */
      ...
      <View className="w-full max-w-screen-sm self-center gap-4 pb-4">
        ...
        {state.status === 'ready' ? (
          <>
            ...
            {/*
              The video-unavailable placeholder and the #11 player seam. It is a
              sibling ABOVE the step list and never gates or disables it.
            */}
            <GuidePlayerPlaceholder sourceUrl={state.guide.sourceUrl} />
      ...
      {state.status === 'ready' ? (
        <FlatList
          ...
          renderItem={({ index, item }) => {
            const detail = state.steps[index];
